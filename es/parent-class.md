...
podéis usar clases externas a KumbiaPHP, como se explica en el próximo
capítulo.
...
Para obtener un manejador de cache se debe utilizar el método «driver» que
proporciona la clase Cache.

###  driver($driver=null)

Este método permite obtener un manejador de cache especifico (APC, file,
...
parámetro $id  con el "id" correspondiente al valor en cache, tomando de
...
Al omitir parámetros al invocar el método save se comporta de la manera
siguiente:
...
ayLoKbZFpNDBDXgXAA)  fue reescrita de forma estática, esto quiere decir ya no
...
Flash es un helper muy útil en Kumbia que permite hacer la salida de mensajes
de error, advertencia, informativos y éxito de forma estándar.
...
Permite enviar un mensaje de éxito al usuario. Por defecto es un mensaje de
...
Permite enviar un mensaje de información al usuario. Por defecto es un mensaje
...
variables de sesión, permitiendo evitar colisiones con nombres de variables.
...
Para poder iniciar una sesión de usuario y realizar la autenticación se debe
invocar el método identify , sin embargo dependiendo del tipo de adaptador, es
necesario especificar ciertos parámetros de configuración.

###  Adaptador Model

Este adaptador permite utilizar autenticación en base a un modelo que herede
de la clase ActiveRecord , verificando los datos de autenticación en la base
de datos.

####  setModel()

Establece el modelo ActiveRecord que se utiliza como fuente de datos. De
manera predeterminada el modelo que se utiliza como fuente de datos es
'users'.
...
Realiza la autenticación. Si ya existe una sesión de usuario activa o los
datos de usuario son correctos, entonces la identificación es satisfactoria.
...
Establece los campos del modelo que se cargaran en sesión mediante el método
...
$algos (string): método de encriptación, el nombre coincide con la función
hash de php.
...
Establece la clave para identificar si existe una sesión autenticada, dicha
clave toma un valor booleano "true" cuando la sesión autenticada es valida,
asignada mediante el método Session::set .
